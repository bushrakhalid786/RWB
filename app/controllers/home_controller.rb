class HomeController < ApplicationController
  ...
  def show_category_page
    if params[:child_alias_name].present?
      ...
      @all_ads = Advertisement.by_categories(category_ids).paginate(page: params[:page],per_page: 30)
    else
      @category = Category.where(alias_name: params[:alias_name] || params[:category]).last
      @all_ads = Advertisement.by_categories([@category.id,@category.child_ids].flatten).paginate(page: params[:page],per_page: 30)
    end
    if @category.present?
